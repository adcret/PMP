...
def main():
    ...
    Img_chi, maximum_chi, minimum_chi, average_chi, TF_chi, row_size_chi, col_size_chi, header_chi = process_data(path, com_chi[6], method='COM')
    Img_phi, maximum_phi, minimum_phi, average_phi, TF_phi, row_size_phi, col_size_phi, header_phi = process_data(path, com_phi[6], method='COM')
    ...
    _, _, num_neighbours = neighbour_rotations(filtered_regions, neighbours_dict, Img_chi, Img_phi)

    if num_neighbours > 1000:
        misorientations = neighbour_misorientation(filtered_regions, neighbours_dict, Img_chi, Img_phi)
        fig, ax1 = plt.subplots(figsize=(8, 6))
        fit_and_plot_chi(misorientations, ax1, 'Misorientations')
    ...
    text = ax[0].text(x_position + scale_bar_length / 2, y_position - scale_bar_thickness * 2, '50 μm', color='black', ha='center', va='top', fontsize=25, fontweight='bold')
    ...
    text = ax[1].text(x_position + scale_bar_length / 2, y_position - scale_bar_thickness * 2, f'{scale_bar_physical_length} μm', color='black', ha='center', va='top', fontsize=25, fontweight='bold')
    text.set_path_effects([path_effects.Stroke(linewidth=3, foreground='white'), path_effects.Normal()])
